...
IMPORTANT: * Astra Control Center와 Cloud Insights가 연결되지 않은 경우 * 클러스터를 관리하지 않으면 원격 측정 데이터를 전송하기 위해 설치된 모든 리소스가 제거됩니다. * Astra Control Center와 Cloud Insights가 연결된 경우 * 클러스터를 관리하지 않으면 만 삭제됩니다 `fluentbit` 및 `event-exporter` Pod를 클릭합니다.
